...
interval=interval,
auto_adjust=True, # Ajuste automatiquement les prix
progress=False
)
df = df.rename(columns=str.title) # Met "Close", "Open", etc. en majuscule
return df
...
# Assurer que "Close" existe
if "Close" not in data.columns:
st.error(f"Colonne 'Close' non trouvée dans les données. Colonnes disponibles : {list(data.columns)}")
else:
